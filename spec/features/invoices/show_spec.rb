...
RSpec.describe 'invoices show' do
  before :each do
    ...
    @test_discount = @merchant1.bulk_discounts.create!(threshold: 9999, percentage: 0)
  ...
  it 'displays total revenue with and without discounts' do 
    ...
    expect(page).to have_content("Total revenue for merchant (before discounts): 162")
    expect(page).to have_content("Total revenue for merchant (after discounts): 81")
  end

  it 'displays link to view discount for invoice item when applicable' do 
    bulk_discount2 = @merchant1.bulk_discounts.create!(percentage: 90, threshold: 11)
